...
use activity_log::{ActivityEvent, ActivityLogAbi, ActivityLogState, ActivityStatus, ActivityTx, Operation};
...
impl MutationRoot {
  ...
  async fn append_event(
    ...
    self.runtime.schedule_operation(&Operation::AppendEvent { actor, event });
    true
  }
  ...
  async fn update_event_status(
    ...
    self.runtime
      .schedule_operation(&Operation::UpdateEventStatus { actor, id, status, tx });
    true
  }
}
...
impl Service for ActivityLogService {
  ...
  async fn handle_query(&self, query: Request) -> Response {
    let state = match ActivityLogState::load(self.runtime.root_view_storage_context()).await {
      Ok(state) => state,
      Err(_) => {
        return Response::from_errors(vec![ServerError::new("storage_error", None)])
      }
    };
